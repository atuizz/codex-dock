...
assert.equal(manifest.data.helper?.version, "0.4.5", "production manifest should expose current Helper version");
assert.match(manifest.data.helper?.sha256 || "", /^[A-F0-9]{64}$/i, "production manifest should expose Helper SHA-256");
assert.ok(manifest.data.helper?.package?.file?.includes("0.4.5"), "production manifest should expose current portable package");
