...
const releaseGates = [
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "safe boundary hold", pattern: /等待 Agent 确认安全轮次边界/ },
      ...
      { label: "fine-grained cloud stages", pattern: /draining_active_turn[\s\S]*payload_issued[\s\S]*writing_auth/ },
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "current Agent version", pattern: /0\.4\.9/ },
...
const uiGates = [
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "Agent version UI", pattern: /Agent 版本分布/ },
      { label: "Agent reconnect UI", pattern: /需重连 Agent|helperStale/ },
      { label: "outdated Agent marker", pattern: /待升级/ },
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "Agent preferred mode", pattern: /helper|本机 Agent/i },
  ...
  },
  {
    ...
    checks: [
      { label: "Agent diagnostics panel", pattern: /helperDiagnostic|Agent 未连接/ },
      { label: "auto-switch stage panel", pattern: /autoSwitchStage|自动切换阶段/ },
      { label: "fine-grained auto-switch stage panel", pattern: /candidate_selecting[\s\S]*payload_issued[\s\S]*writing_auth[\s\S]*restarting_codex[\s\S]*restoring_window/ },
      { label: "Agent update panel", pattern: /检查更新|update/i },
      { label: "Agent portable package action", pattern: /下载 portable 包/ },
    ],
  },
  {
...
await expectText("scripts/smoke-production-surface.mjs", [
  ...
  { label: "self-service delete surface", pattern: /deleteCloudAccountPanel[\s\S]*永久删除云端账号/ },
  { label: "self-service delete API asset check", pattern: /deleteCloudAccountPanel[\s\S]*method:\\s\*"DELETE"/ },
  { label: "admin deletion metric surface", pattern: /24h 注销/ },
...
await expectText("docs/release-and-verification.md", [
  ...
  { label: "current verification evidence", pattern: /## Current Verification Evidence/ },
      { label: "latest Agent release", pattern: /(?:Helper|Agent) `0\.4\.9`/ },
  { label: "candidate versus production report", pattern: /release:report:production/ },
