...
- 已将 Helper 升级到 `0.4.3`，上报版本与构建日期，控制台可识别过旧 Helper；自动切换同类失败连续出现 3 次后会暂停本机自动切换 30 分钟并支持控制台恢复；发布流程见 `.github/workflows/ci.yml` 和 `.github/workflows/cloudflare-deploy.yml`。
