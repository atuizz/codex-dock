...
- Automated production smoke verified on `2026-05-26`: `npm run smoke:production` passed in strict local-helper-hash mode, covering register/login/logout, static asset manifest/version parity, OAuth provider-error handling, structured API error codes/request ids/diagnostic summaries, usage-refresh settings, device registration, non-admin admin rejection, token-free account listing, and Helper download hash parity. Current static asset version is `9e8ec588179c`; production `index.html` has versioned JS/CSS references, the account health center, the cleanup confirmation modal, the Helper release card, the auto-switch stage card, the Helper tray repair buttons, and no stale `20260525-oauth-primary2` string.
