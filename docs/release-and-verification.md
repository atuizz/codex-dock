...
- Windows Helper: `dist/CodexDockHelper/CodexDockHelper.exe`, currently version `0.4.3` with build date `2026-05-26`.
...
- Latest production release verified on `2026-05-26`: `wrangler deploy` published Worker version `1d331f98-a6f4-4108-8b50-994822d3eddb` with static asset version `9e8ec588179c`; production smoke passed in strict local-helper-hash mode; online Helper download and local `dist\CodexDockHelper\CodexDockHelper.exe` both report SHA-256 `FAFC24A50A514EC13B76292D15CEF2980123E17564A0BB709E7189CE4BB30E4B`.
...
- Helper `0.4.3` local release verified on `2026-05-26`: `dist\CodexDockHelper\CodexDockHelper.exe` SHA-256 is `FAFC24A50A514EC13B76292D15CEF2980123E17564A0BB709E7189CE4BB30E4B`, `/api/health` reports `failure_count`, `failure_pause_until`, and `failure_pause_reason`, and the device panel renders the `failure_paused` stage plus `恢复自动切换` action.
...
- Local Helper health verified against `http://127.0.0.1:18766/` with version `0.4.3`, build date `2026-05-26`, active Codex state, `safe_to_switch: false`, tray visibility, and auto-switch failure-pause fields.
- Helper lifecycle regression verified on `2026-05-26`: closing the main window hides to tray, process stays alive, `/api/health` remains available with `tray` diagnostics, `/api/diagnostics/export` returns redacted logs/status, no Microsoft .NET Framework dialog appears, no new `[unhandled:]` log entry is emitted, simulated Windows `TaskbarCreated` restores the tray icon registration, and Helper `0.4.3` silently re-registers `NotifyIcon` even when the main window is visible.
- Production deployment verified on `2026-05-26`: `wrangler deploy` published Worker version `1d331f98-a6f4-4108-8b50-994822d3eddb`, API register/login/logout and usage-refresh settings smoke tests passed, and the deployed static assets use version `9e8ec588179c`.
- Online Helper download verified on `2026-05-26`: `https://codex.woai.pro/downloads/CodexDockHelper.exe` SHA-256 matches local fixed build `FAFC24A50A514EC13B76292D15CEF2980123E17564A0BB709E7189CE4BB30E4B`.
- Automated production smoke verified on `2026-05-26`: `npm run smoke:production` passed in strict local-helper-hash mode, covering register/login/logout, static asset manifest/version parity, structured API error codes/request ids/diagnostic summaries, usage-refresh settings, device registration, non-admin admin rejection, token-free account listing, and Helper download hash parity. Current static asset version is `9e8ec588179c`; production `index.html` has versioned JS/CSS references, the account health center, the cleanup confirmation modal, the Helper release card, the auto-switch stage card, the Helper tray repair buttons, and no stale `20260525-oauth-primary2` string.
...
- Helper release browser verification on `2026-05-26`: production `asset-manifest.json` reports Helper `0.4.3`, build date `2026-05-26`, and SHA-256 `FAFC24A50A514EC13B76292D15CEF2980123E17564A0BB709E7189CE4BB30E4B`; production `panels-ui.js` renders the release card with download and checksum actions. De-identified screenshot: `artifacts/verification/helper-release-card-production.png`.
