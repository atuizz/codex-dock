...
- Latest production release verified on `2026-05-27`: `wrangler deploy` published Worker version `2213af04-9ae2-4016-a691-1a70b7dbbb92` with static asset version `4c1244761f7c`; strict production smoke passed with online/local Helper hash parity; production surface smoke confirmed the commercial modules and no obvious public secret leakage; the online Helper distribution is `0.4.7` with EXE SHA-256 `391F8841D88F1434EAEB144A5435ACF5050AE6CCC4D7F3E5462EF90F74FAC515` and portable zip SHA-256 `87DCA57B1F815CD57FEA5215AC3A0C2462A0F186B6A8831E733C99D977D06B94`.
...
- Production surface smoke refreshed on `2026-05-27`: `npm run smoke:production:surface` verified deployed static asset version `4c1244761f7c`, Helper `0.4.7`, 17 versioned frontend assets, key `app.js`/`styles.css`/`shell-ui.js`/`panels-ui.js`/`settings-ui.js`/`dialog-ui.js`/`admin-ui.js`/`oauth-core.js` surfaces, responsive breakpoints, manual-switch guard, OAuth error copy, and no obvious public secret leakage. Evidence: `artifacts/verification/production-surface-result.json` and `artifacts/verification/helper-update-release-production-result.json`.
