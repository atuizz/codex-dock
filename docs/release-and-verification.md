...
- GitHub release-readiness audit added on `2026-05-26`: the live audit now records whether the current commit has a successful manual CI run, whether a push-triggered CI run actually appeared, whether required GitHub secret names exist, and whether external check suites are still queued. Current evidence is expected to remain `not ready` until `CLOUDFLARE_API_TOKEN` is configured and push-triggered CI is observed for the shipping commit.
