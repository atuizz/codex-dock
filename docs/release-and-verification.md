...
- Production deployment verified on `2026-05-26`: `wrangler deploy` published Worker version `19f41bbd-2771-4fb0-b26f-af6fbcc520f5`, API register/login/logout and usage-refresh settings smoke tests passed, and the deployed static assets use version `edce6b24c714`.
...
- Automated production smoke verified on `2026-05-26`: `npm run smoke:production` passed in strict local-helper-hash mode, covering register/login/logout, static asset manifest/version parity, structured API error codes/request ids/diagnostic summaries, usage-refresh settings, device registration, non-admin admin rejection, token-free account listing, and Helper download hash parity. Current static asset version is `edce6b24c714`; production `index.html` has versioned JS/CSS references, the account health center, the Helper tray repair buttons, and no stale `20260525-oauth-primary2` string.
