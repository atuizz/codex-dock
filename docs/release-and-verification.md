...
- Latest production release verified on `2026-05-26`: `wrangler deploy` published Worker version `cd60b803-0014-4457-99e7-7796a86a813e` with static asset version `9855935fb6d6`; production smoke passed in strict local-helper-hash mode; production surface smoke loaded `app.js?v=9855935fb6d6`, confirmed the commercial UI markers and public assets, found no obvious public secret leakage, and verified online Helper distribution `0.4.6` with EXE SHA-256 `6E936870B63ECC8A8A4C1357D56FDC850B2D9FDA81CD88967EC9D4038CDB90B2`.
...
- Production surface smoke added on `2026-05-26`: `npm run smoke:production:surface` verified deployed static asset version `9855935fb6d6`, Helper `0.4.6`, 17 versioned frontend assets, key `app.js`/`styles.css`/`shell-ui.js`/`panels-ui.js`/`settings-ui.js`/`dialog-ui.js`/`admin-ui.js`/`oauth-core.js` surfaces, responsive breakpoints, manual-switch guard, OAuth error copy, and no obvious public secret leakage. Evidence: `artifacts/verification/production-surface-result.json`.
...
- Production deployment verified on `2026-05-26`: `wrangler deploy` published Worker version `cd60b803-0014-4457-99e7-7796a86a813e`, API register/login/logout and usage-refresh settings smoke tests passed, and the deployed static assets use version `9855935fb6d6`.
...
- Automated production smoke verified on `2026-05-26`: `npm run smoke:production` passed in strict local-helper-hash mode, covering register/login/logout, static asset manifest/version parity, OAuth provider-error handling, structured API error codes/request ids/diagnostic summaries, usage-refresh settings, device registration with Helper `0.4.6`, non-admin admin rejection, token-free account listing, and Helper download hash parity. Current static asset version is `9855935fb6d6`; production `index.html` has versioned JS/CSS references, the account health center, the cleanup confirmation modal, the Helper release card, the auto-switch stage card, the Helper tray repair buttons, and no stale `20260525-oauth-primary2` string.
...
- Helper portable package verification on `2026-05-26`: production `asset-manifest.json` now exposes `downloads/CodexDockHelper-release.json` and `downloads/CodexDockHelper-0.4.6-portable.zip` with SHA-256 `830D363FC4A8CBE471F154BF4BAE5B058DDC06715E18F9CE1546353648D04D89`; production smoke verifies the EXE, zip and release manifest hashes, and browser verification confirms the Helper card renders the `下载 portable 包` action. Evidence: `artifacts/verification/helper-portable-release-production.png`.
