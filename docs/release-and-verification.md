...
- Windows Helper: `dist/CodexDockHelper/CodexDockHelper.exe`, currently version `0.4.6` with build date `2026-05-26`.
...
`npm run release:report` prints a machine-readable evidence summary for the current release candidate: git commit, Worker/static asset version, Helper release manifest, production Helper distribution, local Helper lifecycle self-test, CI/CD workflow coverage, and required verification artifacts.
...
- `npm run release:github-ci` is the controlled CI fallback for the current repository state. It dispatches the CI workflow for the current branch, waits for the current commit's `workflow_dispatch` run, and writes the ignored local evidence file `artifacts/verification/github-ci-dispatch-result.json`. This fallback does not replace push-triggered CI; it keeps release operators from shipping without a GitHub-hosted preflight while the push trigger issue is unresolved.
- `npm run release:github-readiness` is the live GitHub-side readiness gate. It uses `gh` read-only calls to verify active workflows, current-commit CI, a real push-triggered CI run, repository PushEvents, repository secret names, and external check-suite status, then writes the ignored local evidence file `artifacts/verification/github-release-readiness-result.json` without exposing secret values. This check is intentionally not part of `preflight` because it depends on GitHub authentication and repository state.
...
- Latest production release verified on `2026-05-26`: `wrangler deploy` published Worker version `7dc246a5-a4e4-4c31-b9ee-ea897b97c2f8` with static asset version `24a0d58f631b`; production smoke passed in strict local-helper-hash mode; production surface smoke loaded `app.js?v=24a0d58f631b`, confirmed the commercial UI markers and public assets, found no obvious public secret leakage, and verified online Helper distribution `0.4.6` with EXE SHA-256 `6E936870B63ECC8A8A4C1357D56FDC850B2D9FDA81CD88967EC9D4038CDB90B2`.
...
- Production surface smoke added on `2026-05-26`: `npm run smoke:production:surface` verified deployed static asset version `24a0d58f631b`, Helper `0.4.6`, 17 versioned frontend assets, key `app.js`/`styles.css`/`shell-ui.js`/`panels-ui.js`/`settings-ui.js`/`dialog-ui.js`/`admin-ui.js`/`oauth-core.js` surfaces, responsive breakpoints, manual-switch guard, OAuth error copy, and no obvious public secret leakage. Evidence: `artifacts/verification/production-surface-result.json`.
...
- GitHub release-readiness audit added on `2026-05-26`: the live audit now records whether the current commit has a successful manual CI run, whether GitHub recorded the PushEvent, whether a push-triggered CI run actually appeared, whether required GitHub secret names exist, and whether external check suites are still queued. `release:github-ci` provides a controlled manual-dispatch fallback and evidence file, but current readiness is expected to remain `not ready` until `CLOUDFLARE_API_TOKEN` is configured and push-triggered CI is observed for the shipping commit.
...
- Local Helper health verified against `http://127.0.0.1:18766/` with version `0.4.6`, build date `2026-05-26`, active Codex state, `safe_to_switch: false`, tray visibility, lifecycle status, and auto-switch failure-pause fields; `GET /api/update/check` returns latest Helper `0.4.6`, SHA-256, size, download URL, and `update_available: false`; the local status page exposes version, update check, and download links. Evidence: `artifacts/verification/helper-update-local-result.json`, `artifacts/verification/helper-lifecycle-health-local-result.json`, `artifacts/verification/helper-lifecycle-self-test-local-result.json`, and `artifacts/verification/helper-update-local-status-result.json`.
- Helper lifecycle regression verified on `2026-05-26`: closing the main window hides to tray, process stays alive, `/api/health` remains available with `tray` and `lifecycle` diagnostics, `/api/diagnostics/export` returns redacted logs/status, `/api/lifecycle/self-test` verifies the log fact source, simulates RichTextBox log-view failure, confirms `log_view_fault_recovered: true`, no Microsoft .NET Framework dialog appears, no new `[unhandled:]` log entry is emitted, simulated Windows `TaskbarCreated` restores the tray icon registration, and Helper `0.4.6` silently re-registers `NotifyIcon` even when the main window is visible.
...
- Helper release browser verification on `2026-05-26`: production `asset-manifest.json` reports Helper `0.4.6`, build date `2026-05-26`, size `178688`, and SHA-256 `6E936870B63ECC8A8A4C1357D56FDC850B2D9FDA81CD88967EC9D4038CDB90B2`; production `panels-ui.js` renders the release card with download, local update check, and checksum actions. De-identified screenshots and evidence: `artifacts/verification/helper-release-card-production.png`, `artifacts/verification/helper-update-release-production.png`, `artifacts/verification/helper-update-release-production-result.json`, and `artifacts/verification/helper-update-release-production-console.log`.
- Helper portable package verification on `2026-05-26`: production `asset-manifest.json` now exposes `downloads/CodexDockHelper-release.json` and `downloads/CodexDockHelper-0.4.6-portable.zip` with SHA-256 `780F6A5FBBEAA5DBF0721A013F1B3C34F99BB8CB8321968B9BA50C2F1E1E9C45`; production smoke verifies the EXE, zip and release manifest hashes, and browser verification confirms the Helper card renders the `下载 portable 包` action. Evidence: `artifacts/verification/helper-portable-release-production.png`.
