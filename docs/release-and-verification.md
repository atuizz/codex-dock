...
- Commercial release gate added on `2026-05-26` and extended on `2026-05-27`: `scripts/verify-commercial-release-gate.mjs` checks 17 named commercial gates and 28 tracked evidence artifacts, including Helper restart revalidation evidence, and is auto-discovered by `scripts/run-local-verifiers.mjs` during `npm run verify` and `npm run preflight`.
