...
- Windows Helper: `dist/CodexDockHelper/CodexDockHelper.exe`, currently version `0.4.5` with build date `2026-05-26`.
...
- Commercial release gate added on `2026-05-26`: `scripts/verify-commercial-release-gate.mjs` checks 16 named commercial gates and 25 tracked evidence artifacts, and is auto-discovered by `scripts/run-local-verifiers.mjs` during `npm run verify` and `npm run preflight`.
- Release evidence report added on `2026-05-26`: `npm run release:report` emits a machine-readable summary for git commit, Cloudflare Worker/static asset version, Helper release manifest, production Helper distribution, lifecycle self-test, CI/CD workflow coverage, and required evidence artifacts; `npm run verify` now runs 31 verifier scripts including `scripts/verify-release-evidence-report.mjs`.
...
- Local Helper health verified against `http://127.0.0.1:18766/` with version `0.4.5`, build date `2026-05-26`, active Codex state, `safe_to_switch: false`, tray visibility, lifecycle status, and auto-switch failure-pause fields; `GET /api/update/check` returns the Helper release version, SHA-256, size, download URL, and `update_available`; the local status page exposes version, update check, and download links. Evidence: `artifacts/verification/helper-update-local-result.json`, `artifacts/verification/helper-lifecycle-health-local-result.json`, `artifacts/verification/helper-lifecycle-self-test-local-result.json`, and `artifacts/verification/helper-update-local-status-result.json`.
- Helper lifecycle regression verified on `2026-05-26`: closing the main window hides to tray, process stays alive, `/api/health` remains available with `tray` and `lifecycle` diagnostics, `/api/diagnostics/export` returns redacted logs/status, `/api/lifecycle/self-test` verifies the log fact source and schedules log-view recovery, no Microsoft .NET Framework dialog appears, no new `[unhandled:]` log entry is emitted, simulated Windows `TaskbarCreated` restores the tray icon registration, and Helper `0.4.5` silently re-registers `NotifyIcon` even when the main window is visible.
- Production deployment verified on `2026-05-26`: `wrangler deploy` published Worker version `e1ffdbc4-2bfd-4d10-922b-f2aee13b9f9b`, API register/login/logout and usage-refresh settings smoke tests passed, and the deployed static assets use version `9855935fb6d6`.
- Online Helper download verified on `2026-05-26`: `https://codex.woai.pro/downloads/CodexDockHelper.exe` SHA-256 matches local fixed build `8BD4576C05126F5BBC949244214A77BB7F29518511BCC9A1BFB2AC28CDEE96E2`.
- Automated production smoke verified on `2026-05-26`: `npm run smoke:production` passed in strict local-helper-hash mode, covering register/login/logout, static asset manifest/version parity, OAuth provider-error handling, structured API error codes/request ids/diagnostic summaries, usage-refresh settings, device registration with Helper `0.4.5`, non-admin admin rejection, token-free account listing, and Helper download hash parity. Current static asset version is `9855935fb6d6`; production `index.html` has versioned JS/CSS references, the account health center, the cleanup confirmation modal, the Helper release card, the auto-switch stage card, the Helper tray repair buttons, and no stale `20260525-oauth-primary2` string.
